...
class TestUserController(BaseTestCase):
    ...
    def test_create_user_method_handles_user_creation_with_duplicate_email_id(self, mock_request_params):
        with self.app.app_context():
            user = UserFactory(email="-LXTuXlk2W4Gskt8KTte", password="Complexx@34")
            ...
            self.assertEqual(
                ...
                "User with email '{}' already exists".format(user.slack_id)
            )
    ...
    def test_list_user_succeeds(self):
        ...
        with self.app.app_context():
            ...
            response = user_controller.list_user(email=user.email)
    ...
    def test_list_user_when_user_found_succeeds(self):
        with self.app.app_context():
            ...
            response = user_controller.list_user(email="-LXTuXlk2W4Gskt8KTtedhmdydsbnyw")
